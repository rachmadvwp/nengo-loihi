...
def core_cx_profiles(core):
    ...
    def list_cx_profiles(block):
        profiles = []
        for i in range(block.compartments.n_compartments):
            profiles.append(CxProfile(
                decayU=block.compartments.decayU[i],
                decayV=block.compartments.decayV[i],
                refractDelay=block.compartments.refractDelay[i],
                enableNoise=block.compartments.enableNoise[i],
            ))
...
def core_vth_profiles(core):
    ...
    def list_vth_profiles(block):
        profiles = []
        vth, _ = vth_to_manexp(block.compartments.vth)
        for i in range(block.compartments.n_compartments):
...
def one_to_one_allocator(model):
    ...
    for block in model.blocks:
        if block.compartments.n_compartments > 1024:
