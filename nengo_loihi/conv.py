...
def build_conv2d_connection(model, conn):
    ...
    synapses = CxSynapses(np.prod(input_shape.spatial_shape),
    ...
    ax = CxAxons(np.prod(input_shape.spatial_shape), label="conv2d_weights")
    ax.target = synapses
        ...
        if weight_key not in weights_map:
            ...
            if transform.channels_last:
                ...
                inds = (
                        ...
                        + transform.n_filters * np.arange(wmask_j.sum())[:,
                                                None]
